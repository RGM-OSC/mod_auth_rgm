 ...
 * Renamed mod_auth_rgm as it is targeted to be used by Eyes of Network appliance
 * Jeremie Bernard <gremimail@gmail.com> - Apr 2016
 */

#define MAE_VERSION 	"mod_auth_rgm 5.0"
...
// Structures to hold the mod_auth_rgm's configuration directives Per-Directory Configuration
typedef struct 
{
...
static void 		auth_rgm_register_hooks_cb	(apr_pool_t *p);
static void *		auth_rgm_create_dir_config_cb	(apr_pool_t *p, char *d);
static int 		auth_rgm_check_user_id_cb	(request_rec *r);
static int 		auth_rgm_session_checker_cb 	(request_rec *r);
static unsigned char *	auth_rgm_check_session		(request_rec *r, auth_rgm_dir_config *config, MYSQL *db_handle, int *expired);
static void 		auth_rgm_track_request		(request_rec *r, auth_rgm_dir_config *config, MYSQL *db_handle, const unsigned char *uid);
static void 		auth_rgm_set_cgi_env_directory	(request_rec *r, const unsigned char *uid);

// Utility functions
static int 		auth_rgm_open_db		(request_rec *r, auth_rgm_dir_config *config, MYSQL **db_handle);
static void 		auth_rgm_close_db		(MYSQL **db_handle);
static q_result *	auth_rgm_send_query_db		(request_rec *r, MYSQL *db_handle, unsigned char *query_format, ... );
static int 		auth_rgm_redirect		(request_rec *r, auth_rgm_dir_config *config, const unsigned char *page, int log_level, 
								const unsigned char *reason_format, ... );
static unsigned char *	auth_rgm_parse_condition_vars	(request_rec *r, const unsigned char *condition, int cookies);
static unsigned char *	auth_rgm_get_value		(request_rec *r, const unsigned char *key_values, const unsigned char *key, unsigned char terminator, 
								const unsigned char *padding);
static unsigned char *	auth_rgm_construct_full_uri	(request_rec *r);
static unsigned char *	auth_rgm_url_encode		(request_rec *r, const unsigned char *uri);
static unsigned char *	auth_rgm_url_decode		(request_rec *r, const unsigned char *uri_enc);

// Default User Option
const 		char * 	auth_rgm_default_user		();

typedef struct {
	const	char *	defaultUser;			
} auth_rgm_config;

static auth_rgm_config global_config;
...
// module entry point
module AP_MODULE_DECLARE_DATA auth_rgm_module =
{
	STANDARD20_MODULE_STUFF,
	auth_rgm_create_dir_config_cb,
	NULL,			
	NULL,		
	NULL,	
	auth_rgm_cmds,
	auth_rgm_register_hooks_cb
};

// CALLBACK: tell Apache which function does what
static void auth_rgm_register_hooks_cb (apr_pool_t *p) 
{
	ap_hook_check_user_id 	(auth_rgm_check_user_id_cb, NULL, NULL, APR_HOOK_REALLY_FIRST);
	ap_hook_auth_checker  	(auth_rgm_session_checker_cb, NULL, NULL, APR_HOOK_REALLY_FIRST);
} 

// Set mod_auth_rgm's environment variables for CGI scripts for the current request.
static void auth_rgm_set_cgi_env_directory (request_rec *r, const unsigned char *uid) 
{
...
static void * auth_rgm_create_dir_config_cb (apr_pool_t *p, char *d) 
{
	auth_rgm_dir_config *config = (auth_rgm_dir_config *) apr_pcalloc (p, sizeof(auth_rgm_dir_config));
...
const char *auth_rgm_default_user(cmd_parms *cmd, void *cfg, const char *arg)
{
	if (arg) 
	{
...
static int auth_rgm_check_user_id_cb (request_rec *r) 
{
	auth_rgm_dir_config *config = (auth_rgm_dir_config *) ap_get_module_config (r->per_dir_config, &auth_rgm_module);
...
static int auth_rgm_session_checker_cb (request_rec *r) 
{
	...
	if (!auth_rgm_open_db (r, config, &db_handle))
		return HTTP_FORBIDDEN;

	r->unparsed_uri = auth_rgm_construct_full_uri (r);

	// Check the session keys against the database using a given condition
	uid = auth_rgm_check_session (r, config, db_handle, &expired);
	...
	{
		// expiration
		status = auth_rgm_redirect (r, config, config->pageExpired, APLOG_INFO, "session expired");
	}
 	else	
	{
		// no corresponding uid found
    		status = auth_rgm_redirect (r, config, config->pageLogin, APLOG_INFO, "invalid session");
	}

	auth_rgm_close_db (&db_handle);
	...
	if (status == OK)
	{
		...
		auth_rgm_set_cgi_env_directory (r, uid);
	}
...
static unsigned char * auth_rgm_check_session (request_rec *r, auth_rgm_dir_config *config, MYSQL *db_handle, int *expired) 
{
	unsigned char *uid = NULL, *query, *condition = auth_rgm_parse_condition_vars (r, config->dbTableSIDCondition, config->sessionCookies);
	...
	*expired = FALSE;
	query = apr_pstrcat (r->pool,
  	...
  	rows = auth_rgm_send_query_db (r, db_handle, query);
    		...
    		if (autorefresh > 0) 
		{
          		...
          		apr_pstrcat (r->pool,
            			apr_psprintf (r->pool, "%d", autorefresh+1),
            				...
            				"=":"",
            				(add_last_page_key)?
            				(char *)auth_rgm_url_encode(r, r->unparsed_uri):"",
            			NULL));
    		}
    		if (config->sessionTimeout > 0)
      			auth_rgm_send_query_db (r, db_handle, "UPDATE %s SET %s=DATE_ADD(NOW(), INTERVAL %d MINUTE) WHERE %s", 
				config->dbTableSID, config->dbFieldTimeout, config->sessionTimeout, condition);

    		if (config->dbTableTracking)
      			auth_rgm_track_request (r, config, db_handle, uid);
  	}
    	// either the session has expired and/or the condition is not met
  	else if (config->sessionTimeout > 0 || config->dbFieldExpiration) 
	{
    		rows = auth_rgm_send_query_db (r, db_handle, "SELECT %s FROM %s WHERE (%s)", 
	...
	// remove all expired sessions
  	if (config->sessionDelete)	
    		auth_rgm_send_query_db (r, db_handle, "DELETE FROM %s WHERE %s",
        		config->dbTableSID,
        		apr_pstrcat(r->pool,
          			(s_timeout_enabled)?
          			(char *)config->dbFieldTimeout:"",
...
static void auth_rgm_track_request (request_rec *r, auth_rgm_dir_config *config, MYSQL *db_handle, const unsigned char *uid) 
{
  	...
  	if (config->dbTableTrackingCondition)
    		trackingCondition_parsed = auth_rgm_parse_condition_vars (r, config->dbTableTrackingCondition, config->sessionCookies);
  	...
  	if (config->trackingLifetime > 0) 
	{
    		if (trackingCondition_parsed)
      			auth_rgm_send_query_db (r, db_handle, "DELETE FROM %s WHERE %s='%s' AND DATE_ADD(%s, "
	...
	// Check for an existing tracking record and update it (both UID and download path must match)
  	if (trackingCondition_parsed)
    		rows = auth_rgm_send_query_db (r, db_handle, "SELECT %s FROM %s WHERE %s='%s' AND %s='%s' AND (%s)",
    		...
    		rows = auth_rgm_send_query_db (r, db_handle, "SELECT %s FROM %s WHERE %s='%s' AND %s='%s'",
        				config->dbFieldUID, config->dbTableTracking, config->dbFieldUID, uid, config->dbFieldDownloadPath, 
					r->unparsed_uri);
	...
	{ 
    		if (trackingCondition_parsed)
      			auth_rgm_send_query_db (r, db_handle, "UPDATE %s SET %s='%s',%s=NOW(),%s='%ld' WHERE %s='%s' "
	...
	// create a new tracking record
    		...
    		auth_rgm_send_query_db (r, db_handle, "INSERT INTO %s (%s, %s, %s, %s, %s) VALUES ('%s', '%s', "
...
static int auth_rgm_redirect (request_rec *r, auth_rgm_dir_config *config, const unsigned char *page, int log_level, const unsigned char *reason_format, ... ) 
{
  	va_list arg_list;
  	...
  	if (page) 
	{
    		...
    		if (config->lastPageKey) 
		{
				...
				auth_rgm_url_encode (r, r->unparsed_uri), (fragment)? (char*)fragment:"", NULL);
    		}
    		else
			...
			// Variable mode
        			...
        			else 
				{
          				...
          				if (condition[chr] != ' ') 
					{
          				...
          				var[vchr] = '\0';
          				if (key_values != NULL)
            					value = auth_rgm_get_value (r, key_values, var, terminator, padding);
          				if (value == NULL)
            					value = "";
          				else
...
static unsigned char * auth_rgm_get_value (request_rec *r, const unsigned char *key_values, const unsigned char *key, unsigned char terminator, const unsigned char *padding) 
{
  	if (key_values && key) 
	{
    		...
    		if (key_begin != NULL) 
		{
      			...
      			return auth_rgm_url_decode (r, value);
    		}
  	}
